...
class OAuthGoogleDriveService:
    """OAuth-based Google Drive service for user authentication"""
    
    def __init__(self, user_access_token: str, user_refresh_token: str):
    ...
    async def ensure_geo_folder(self, geo: str) -> str:
        """Ensure GEO folder exists in user's Drive and return its ID"""
    ...
    async def upload_file(
        ...
        """
        logger.info(f"Starting OAuth file upload: {filename} ({len(file_content)} bytes) to GEO: {geo}")
        ...
        try:
            ...
            logger.info("Starting OAuth Google Drive API upload...")
            ...
            logger.info(f"OAuth file uploaded successfully!")
            ...
            logger.error(
                "Failed to upload file via OAuth",
                filename=filename,
                geo=geo,
                error=str(e)
            )
